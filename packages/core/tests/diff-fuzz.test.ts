...
const applyListLikeChanges = (oldState: string[], changes: readonly Change[]) => {
    ...
    for (const c of changes) {
        ...
        if (c.kind === "insert" || c.kind === "insert-container") {
            ...
            if (typeof c.value !== "string") {
                throw new Error(`Expected string value, got ${typeof c.value}`);
            }
...
describe("diff fuzz", () => {
    it("diffListWithIdSelector: fuzz correctness + minimal delete/insert counts", () => {
        ...
        for (let round = 0; round < ROUNDS; round++) {
            ...
            const { deletes, inserts, moves } = countKinds(changes);
            expect(moves).toBe(0);
