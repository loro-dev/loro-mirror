...
describe("Mirror - State Consistency", () => {
    ...
    it("handles nested container updates", async () => {
        ...
        // Create the posts list as a nested container under blog to avoid
        // creating unrelated root-level containers that aren't in the schema
        const postsList = blogMap.setContainer("posts", new LoroList());

        // Create first post as a nested map container (not a root-level map)
        const post1 = postsList.insertContainer(0, new LoroMap());
        post1.set("id", "1");
    ...
    it("handles recursive schemas", async () => {
        ...
        expect(
            valueIsContainerOfType(serialized.root.value.name, ":Text"),
        ).toBe(true);
        ...
        expect(
            valueIsContainerOfType(
                serialized.root.value.children.value[0],
                ":Map",
            ),
        ).toBe(true);
    ...
    it("should not override LoroDoc data even if initState is provided", async () => {
        const someState = {
            list: [{}],
            text: "some string",
        };
