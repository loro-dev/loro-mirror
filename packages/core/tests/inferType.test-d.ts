...
import type { LazyList } from "../src/index.js";
...
describe("lazy list types", () => {
    const lazyListSchema = schema.LoroList(
        schema.LoroMap({
    ...
    test("InferType of a lazy LoroList is LazyList", () => {
        type Item = {
            id: string;
            title: string;
            done: boolean;
        ...
        type InferredType = InferType<typeof lazyListSchema>;

        expectTypeOf<InferredType>().toEqualTypeOf<
            LazyList<Item, Partial<Item>>
        >();
    });

    test("InferInputType of a lazy LoroList is LazyList (arrays rejected)", () => {
        type Item = {
            id: string;
            title: string;
            done: boolean;
        ...
        type InputType = InferInputType<typeof lazyListSchema>;

        expectTypeOf<InputType>().toEqualTypeOf<
            LazyList<Item, Partial<Item>>
        >();
        expectTypeOf<unknown[]>().not.toMatchTypeOf<InputType>();
    });

    test("non-lazy LoroList keeps array types (control)", () => {
        const plainList = schema.LoroList(
            schema.LoroMap({ id: schema.String({ required: true }) }),
            (it) => it.id,
        );

        type Item = { id: string } & { $cid: string };

        expectTypeOf<InferType<typeof plainList>>().toEqualTypeOf<Item[]>();
        expectTypeOf<InferInputType<typeof plainList>>().toEqualTypeOf<
            Array<{ id: string } & { $cid?: string }>
        >();
    });
});
